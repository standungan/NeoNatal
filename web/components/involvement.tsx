...
/** Short axis labels for the 6 Pillar-6 items (keyed by item_code). */
export const ITEM_SHORT: Record<string, string> = {
  keluarga_1: "ASI",
  keluarga_2: "Paham Kondisi",
  keluarga_3: "PMK",
  keluarga_4: "Perawatan Dasar",
  keluarga_5: "Edukasi",
  keluarga_6: "Perawatan Rumah",
};
...
/** Spider/radar chart of the 6 "Kerjasama dengan Keluarga" items (0–3 scale).
 *  Shared by the involvement entry form (live) and the report page (latest record). */
export function InvolvementRadar({ items, height = 280 }: { items: RadarItem[]; height?: number }) {
...
/** Latest-involvement summary: score + category + per-item bars + alarms + radar.
 *  Used on the report page. */
export function InvolvementSummaryCard({
  record,
  title = "Keterlibatan Orang Tua",
}: {
  record: InvolvementRecord;
  title?: string;
}) {
  return (
    <Card className="p-5">
      ...
      <p className="mt-0.5 text-[11px] text-muted">Pilar 6 — Kerjasama dengan Keluarga</p>
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <InvolvementRadar items={record.items} />
      </div>
    </Card>
  );
}
